...
#define C2_STRINGIFY(x) C2_STRINGIFY_IMPL(x)  // NOLINT
    if (CHATTERINO_GIT_NIGHTLY_COMMITS > 0)
    {
        this->buildString_ += '-';
        this->buildString_ += u"" C2_STRINGIFY(CHATTERINO_GIT_NIGHTLY_COMMITS);
    }
    if (CHATTERINO_GIT_FORK_COMMITS > 0)
    {
        this->buildString_ += '+';
        this->buildString_ += u"" C2_STRINGIFY(CHATTERINO_GIT_FORK_COMMITS);
    }
